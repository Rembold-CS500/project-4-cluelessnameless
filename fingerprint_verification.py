...
def shifted_check(fingerprint1, fingerprint2, threshold = .95):
    ...
    for dx in range(-len(fingerprint1['fingerprint']), len(fingerprint1['fingerprint'])):
        for dy in range(-len(fingerprint1['fingerprint'][dx]), len(fingerprint1['fingerprint'][dx])):
            matches = 0
            total = 0
...
"""
    ...
    # expected True - was returning False but why?
    ...
    # expected False - now this returns True but why?
"""